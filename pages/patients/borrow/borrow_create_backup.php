...
<div class="wrapper">
    ...
    <?php
    // SQL Crate borrow
    $sql_borrow = ("SELECT * FROM tbl_patient
 WHERE hn_patient = :hn_patient ");
    $stmt_borrow = $connection->prepare($sql_borrow);
    $stmt_borrow->execute(array(":hn_patient" => $_GET['hn_borrow']));
    $row_borrow = $stmt_borrow->fetch(PDO::FETCH_OBJ);
    // SQL PSG --------------------------
    $sql_psg = ("SELECT * FROM tbl_psg
     WHERE psg_hn = :hn_patient ");
    $stmt_psg = $connection->prepare($sql_psg);
    $stmt_psg->execute(array(":hn_patient" => $_GET['hn_borrow']));
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card shadow">
                            <div class="card-header bborrow-0 pt-4">
                                <h3 class="card-title">
                                    <i class="fas fa-cart-arrow-down"></i>
                                    รายการยืมอุปกรณ์ของ<?php echo "คุณ" . $row_borrow->firstname . "  " . $row_borrow->lastname. "  " . "สิทธิการรักษา" . $row_borrow->claim; ?>
                                </h3>
                                <br>
                            </div>
                            <!-- <form id="form_add_borrow"> -->
                                <div class="card-body">
                            <!-- <?php    print_r($row_psg);   ?> -->
                                <div class="row">
                                        <div class="col-3">
                    ...
                    <div class="col-12">
                        <div class="card shadow">
                            <div class="card-header bborrow-0 pt-4">
                                ...
                                <!-- <a href="print_borrow.php?hn_borrow=<?php echo $hn_borrow ; ?>" class="btn btn-warning mt-3" >
	<i class="fas fa-user"></i> Print ใบสั่งซื้อ </a> -->
                            ...
                            <div class="card-body">
                                <div class="table-responsive-xxl">
                                    <table class="table table-hover" id="borrows">
                                        <thead>
...
<?php include_once 'borrow_create_script.php'; ?>
</body>
